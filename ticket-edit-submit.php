...
<div id="container" class="center-block pager" style="width:90%;">
    <div id="content" class="center-block" style="width:80%;">
        ...
        if ($ticket_id_to=='default') {
            $sql = mysql_query("UPDATE Tickets SET Time_Bought='$date_updated', is_earlybird='$is_early', is_Sold=$is_sold, is_Student_Price=$is_student, Paid_To='$admin_username', Sold_By='$sold_by', Details='$end_buyer' WHERE Ticket_Number=$ticket_id_fr");
        } else {
            $sql = mysql_query("UPDATE Tickets SET Time_Bought='$date_updated', is_earlybird='$is_early', is_Sold=$is_sold, is_Student_Price=$is_student, Paid_To='$admin_username', Sold_By='$sold_by', Details='$end_buyer' WHERE (Ticket_Number >= $ticket_id_fr AND Ticket_Number <= $ticket_id_to)");
        }
